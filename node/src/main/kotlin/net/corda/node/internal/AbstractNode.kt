...
abstract class AbstractNode(open val configuration: NodeConfiguration,
    ...
    private fun makeInfo(legalIdentity: PartyAndCertificate): NodeInfo {
        val notaryEntry = getNotaryEntry() // TODO keep only notary, rest don't care
        val allIdentitiesList = mutableListOf(legalIdentity)
        if (notaryEntry != null) allIdentitiesList.add(notaryEntry.identity) // TODO Will we keep service identities here, for example notaries?
     ...
     */
    // TODO Now I make entries only for notaries
    protected open fun getNotaryEntry(): ServiceEntry? {
        return advertisedServices.singleOrNull { it.type.isNotary() }?.let {
            myNotaryIdentity = obtainIdentity(it)
            ServiceEntry(it, myNotaryIdentity)
        }
    }
    ...
    // TODO add notary to serviceHub notaryKey
    ...
    private fun obtainIdentity(serviceInfo: ServiceInfo? = null): PartyAndCertificate {
        ...
        val (id, name) = if (serviceInfo == null) {
            ...
            // TODO if simple -> add simple to name! (first if)
    ...
    private inner class ServiceHubInternalImpl : ServiceHubInternal, SingletonSerializeAsToken() {
        ...
        override val notaryIdentity: PartyAndCertificate get() = myNotaryIdentity// TODO IllegalStateException
