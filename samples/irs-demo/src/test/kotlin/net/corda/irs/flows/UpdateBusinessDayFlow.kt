...
object UpdateBusinessDayFlow {
    ...
    class Broadcast(val date: LocalDate, override val progressTracker: ProgressTracker) : FlowLogic<Unit>() {
        ...
        private fun getRecipients(): Iterable<Party> {
            val notaryParties = serviceHub.networkMapCache.notaryIdentities.map { it.party } // TODO Will break on distributed nodes, but it will change after services removal.
            // TODO REWRITE
            val peerParties = serviceHub.networkMapCache.partyNodes.filter { it.legalIdentities.all { it !in notaryParties } }.map { it.legalIdentities.first() }.sortedBy { it.name.toString() }//(serviceHub.networkMapCache.partyNodes.map { it.legalIdentitiesAndCerts.first().party } - notaryNodes).sortedBy { it.name.toString() }
