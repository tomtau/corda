...
class NewTransaction : Fragment() {
    ...
    init {
        ...
        partyBChoiceBox.apply {
            ...
            items = FXCollections.observableList(parties.map { it.chooseIdentityAndCert() }).sorted() // TODO check
...
//            items = issuers.map { it.identity.party }.unique().sorted()
            items = issuers.map { it.chooseIdentity() }.unique().sorted() // TODO  fix that
